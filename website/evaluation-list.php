		...
		<div class="sh-mainpanel">
			<div class="sh-breadcrumb">
				<nav class="breadcrumb">
					<a class="breadcrumb-item" href="index.html">Evaluation</a>
			...
			<div class="sh-pagebody" ng-app="homeApp" ng-controller="formCtrl" ng-init="init()">
				<div class="list-data">
						...
						<table class="table-listing">
							...
							<tbody>
								<tr ng-repeat="audio in data track by $index">
									...
									<td>{{audio.l_name}}</td>
									<td>{{getLanguage(audio.language)}}</td>
									<td>{{audio.level}}</td>
									<td>
										<button class="btn btn-primary" ng-click="showEvaluation(audio)">Show </button>
									</td>
									<td>
										<button class="btn btn-danger" ng-click="deleteEva($index)">Delete</button>
									</td>
								</tr>
							</tbody>
						</table>
			 	...
			 	<!-- ./list-data -->
			 	<?php include("modal/feedback-list/evaluation-audio.php"); ?>
			</div><!-- sh-pagebody -->
		</div><!-- sh-mainpanel -->
...
<script>
	...
	app.controller('formCtrl', function($scope, $http, $timeout) {
		...
		$scope.getData = function(page) {
			var url = "ajax/audio_feedback.php?entry="+$scope.dataEntry+"&page="+page;
		...
		$scope.countData = function(count) {
			var url = "ajax/audio_feedback.php?count";
			angular.forEach($scope.filter, function(value, key) {
				if (value.trim() != "") {
					url += "&"+key+"="+value;
				}
		...
		$scope.deleteEva = function(index) {
			var data = $scope.data[index];
			var id = data._id.$id;
			if(confirm('Are you sure to delete audio name '+id) == false) {
				return 0;
			}
		...
		$scope.getLanguage = function(key) {
			var language = {
				"en": "English",
				"fr": "France"
			}
		...
		$scope.showEvaluation = function(data) {
			var eva = angular.copy(data.evaluation);
			$scope.evaluation = eva;
			$scope.setModal("evaluation-audio");
		}

		// set Modal popup and close
